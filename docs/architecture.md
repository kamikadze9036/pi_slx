...
For the factory VM, `MES_PROVIDER=euromap63` bypasses the KPI provider and queries only the existing Euromap63 Docker API. The plant overview reads `/api/machines/status` and `/api/collectors/health`. Shift screens read `/api/machines` to map Cyclades IDs to Euromap machine codes, then bounded `/api/cycles` and `/api/downtimes` for the selected shift. If no individual cycles exist, they read `/api/cycles/cyclades-derived` and sum nonnegative increases between periodic counter samples by hour. Missing intervals and counter resets are excluded; current machine state is shown separately from historical shift data. The backend does not turn cycles or counter increases into good pieces or compute scrap, OEE, availability, performance, or quality without validated inputs. Euromap63 itself may obtain machine state, counters and stop reasons from Cyclades; `pi_slx` has no direct SQL connection in this mode.
